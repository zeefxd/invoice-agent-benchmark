...
def calculate_totals(lines: list) -> dict:
    ...
    net = round(sum(line.get("net_total", 0) for line in lines), 2)
    vat = round(sum(line.get("vat_amount", 0) for line in lines), 2)
    gross = round(sum(line.get("gross_total", 0) for line in lines), 2)
