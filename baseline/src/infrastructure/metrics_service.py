...
class MetricsService:
    def __init__(self, model: str, framework: str = "baseline-python"):
        self.model = model
    ...
    def save(self):
        ...
        with open(filename, "w", encoding="utf-8") as out_file:
            json.dump(self.to_dict(), out_file, indent=2, ensure_ascii=False)

        header = (
            "framework,model,ttfr_s,total_time_s,llm_rounds,tokens_input,tokens_output,"
            "tokens_total,ram_peak_mb,auto_score,invoice_json_ok,timestamp\n"
        )
        ...
        row = (
            f"{self.framework},{self.model},{result['ttfr_s']},{result['total_time_s']},"
            f"{self.llm_rounds},{self.tokens_input},{self.tokens_output},{result['tokens_total']},"
            f"{result['ram_peak_mb']},{quality.get('auto_score_0_5', 0)},"
            f"{1 if result.get('final_invoice_json') else 0},{result['timestamp']}\n"
        )

        with open(SUMMARY_CSV_PATH, "a", encoding="utf-8") as summary_file:
